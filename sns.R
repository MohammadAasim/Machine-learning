...
teens$age <- ifelse(teens$age >= 1 & teens$age < 20, teens$age, NA)
summary(teens$age) #more missing data

teens$female <- ifelse(teens$gender == "F" & !is.na(teens$gender), 1, 0)
#if gender is NA, it will assign 1 otherwise 0
...
#used to find average age for every year after removing all the NA values
...
#scale to resale features to get mean of 0 and std deviation of 1
set.seed(2345)
...
#model performance
teen_clusters$size
#show the cluster size of 5 clusters. so many teenagers have same interest so to verify find the centroid
teen_clusters$centers
#to find the centroid. some values are more than mean and below mean and some are below mean
#improve performance
